...
=======
        ...
        try {
          ...
          // Store user data including student profile data
          const storage = rememberMe ? localStorage : sessionStorage;
          storage.setItem("userId", authData.user?.id || "");
          storage.setItem("userEmail", authData.user?.email || "");
          storage.setItem("userRole", "student");
          storage.setItem("studentData", JSON.stringify(studentData));
          storage.setItem("authToken", authData.session?.access_token || "");
          console.log("User and student data stored in storage");
          ...
          console.error("Error in student data handling:", fetchError);
          setError(fetchError.message || "Could not access your student profile");
          setIsLoading(false);
        }

      } else {
        // If it's a CET ID, we need to look up the associated email
        try {
          // First, query the students table to find the user by CET ID
          const { data: studentData, error: studentError } = await supabase
            .from('students')
            .select('email, id')
            .eq('cet_application_id', loginIdentifier)
            .single();

          if (studentError || !studentData) {
            console.error("Error finding CET ID:", studentError);
            throw new Error("Invalid CET ID. Please check and try again.");
          }

          // Now use the found email to log in
          const { data: authData, error: authError } = await supabase.auth.signInWithPassword({
          ...
          if (!fullStudentError) {
            storage.setItem("studentData", JSON.stringify(fullStudentData));
          }
          ...
          console.error("CET ID login failed:", error);
